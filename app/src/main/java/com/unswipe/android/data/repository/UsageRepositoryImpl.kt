...
class UsageRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAppUsageToday(packageName: String): Long {
        if (!hasUsageStatsPermission(context)) {
            return 0L
        }

        val startTime = getStartOfDayInMillis()
        val endTime = System.currentTimeMillis()
        ...
        try {
            ...
            println("Error getting app usage for $packageName: ${e.message}")
            return 0L
        }
    }

    override suspend fun recordAppLaunchAttempt(
        packageName: String,
        wasBlocked: Boolean,
        usageAtTime: Long
    ) {
        try {
            // Create a usage event for the app launch attempt
            val event = UsageEvent(
                id = 0, // Auto-generated by Room
                packageName = packageName,
                eventType = if (wasBlocked) "APP_LAUNCH_BLOCKED" else "APP_LAUNCH_ALLOWED",
                timestamp = System.currentTimeMillis(),
                additionalData = "usageAtTime:$usageAtTime"
            )
            
            usageDao.insertUsageEvent(event)
        } catch (e: Exception) {
            println("Error recording app launch attempt: ${e.message}")
