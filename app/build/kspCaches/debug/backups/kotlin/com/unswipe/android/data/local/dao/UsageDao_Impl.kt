  ...
  init {
    ...
    this.__insertAdapterOfUsageEvent = object : EntityInsertAdapter<UsageEvent>() {
      protected override fun createQuery(): String =
          "INSERT OR REPLACE INTO `usage_events` (`id`,`timestamp`,`packageName`,`eventType`) VALUES (nullif(?, 0),?,?,?)"
    ...
    return createFlow(__db, false, arrayOf("usage_events")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          _item = UsageEvent(_tmpId,_tmpTimestamp,_tmpPackageName,_tmpEventType)
          _result.add(_item)
        }
        _result
      } finally {
